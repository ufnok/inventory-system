...
router.get('/', async (req, res) => {
  try {
    ...
    let list = db.data.tables.check_order;
...
router.get('/:id', async (req, res) => {
  try {
    const order = db.data.tables.check_order.find(o => o.id === req.params.id);
    ...
    const items = db.data.tables.check_item.filter(i => i.orderId === req.params.id).map(i => {
...
router.post('/', async (req, res) => {
  try {
    ...
    db.data.tables.check_order.push({ id: orderId, checkDate: checkDate || new Date().toISOString().slice(0, 10), operatorId: req.user.id, auditorId: null, auditorTime: null, status: 1, remark: remark || '', createdAt: new Date().toISOString() });
    ...
    products.forEach(p => {
      ...
      db.data.tables.check_item.push({ id: Date.now() + Math.random(), orderId, productId: p.id, systemQty, actualQty: systemQty, diffQty: 0, status: 'EQUAL', createdAt: new Date().toISOString() });
    });
    await db.write();
...
router.put('/:id/items', async (req, res) => {
  try {
    ...
    items.forEach(item => {
      const checkItem = db.data.tables.check_item.find(i => i.id === item.id);
      if (checkItem) {
...
router.post('/:id/submit', async (req, res) => {
  try {
    ...
    const items = db.data.tables.check_item.filter(i => i.orderId === req.params.id && i.diffQty !== 0);
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    db.data.tables.check_order = db.data.tables.check_order.filter(o => o.id !== req.params.id);
    db.data.tables.check_item = db.data.tables.check_item.filter(i => i.orderId !== req.params.id);
    await db.write();
