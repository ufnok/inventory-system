<template>
  <div class="page-container">
    <el-card>
      <template #header><span>出库单据列表</span></template>
...
import { stockOutApi, customerApi } from '@/api'
import { ElMessage, ElMessageBox } from 'element-plus'

const list = ref([]), loading = ref(false), detailDialogVisible = ref(false), customerList = ref([])
...
async function loadData() {
  ...
  try {
    ...
    const res = await stockOutApi.list(params); list.value = res.data.list; total.value = res.data.total
